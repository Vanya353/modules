...
const getUsersByEyeColor = (arr, color) => {
  const arrUsers = arr.filter(user => user.eyeColor === color);
  return arrUsers;
};
...
const getUsersByGender = (arr, gender) => {
  const genderMan = arr.filter(user => user.gender === gender);
  return genderMan;
};
...
const getInactiveUsers = arr => {
  const notActive = arr.filter(user => !user.isActive);
  return notActive;
};
...
const getTotalBalance = arr => {
  const sumUsers = arr.reduce((sum,user)=> sum += user.balance, 0);
  return sumUsers;
};
...
const getUsersByFriend = (arr, name) => {
  const friend = arr.filter((user) => user.friends[1] == name || user.friends[0] == name);//треба знайти більш краще рішення.
  return friend;
}
...
const getAllUniqueSkills = arr => {
  const skillsUsers = arr.reduce(
  (sumSkills,user) => user.skills ? sumSkills.concat(user.skills) : sumSkills,[] 
  );
  const uniqueElem = skillsUsers.filter((item,index) => {
    return index === skillsUsers.indexOf(item);
  });
  uniqueElem.sort()
  return uniqueElem;
}
